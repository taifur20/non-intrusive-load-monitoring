...
void sensor_ssss_configure(void)
{
  ...
  SeeedOled.putString(" ModelNumber: ");
  SeeedOled.putNumber(4);           //Print number
  ...
  */
  ...
  qorc_ssi_adc_volt.setMode(0);     //1 = single, 0 - continious
  qorc_ssi_adc_volt.setDataRate(7); //max
  qorc_ssi_adc_volt.readADC(0); //max

  qorc_ssi_adc_amps.begin();
  qorc_ssi_adc_amps.setGain(1);     //+/- 4.096V
  qorc_ssi_adc_amps.setMode(0);     //1 = single, 0 - continious
  qorc_ssi_adc_amps.setDataRate(7); //max
  qorc_ssi_adc_amps.readADC(0); //max
...
int  sensor_ssss_acquisition_buffer_ready()
{
    ...
    //int16_t val_voltage = qorc_ssi_adc.readADC(0) - 20125; //5V input to sensor
    //int16_t val_current = qorc_ssi_adc.readADC(1) - 17800;

    int16_t adc_voltage = qorc_ssi_adc_volt.getValue(); //3.3V input to sensor
    int16_t adc_current = qorc_ssi_adc_amps.getValue();

    int16_t offset_voltage = adc_voltage - 13000;
    int16_t offset_current = adc_current - 13000;

    double converted_voltage = (4.096 / 32768.0) * offset_voltage;
    double converted_current = (4.096 / 32768.0) * offset_current;

    int16_t insVoltage = converted_voltage * 40000;
    int16_t insCurrent = converted_current * 10000;
    ...
    for(int i=0; i<no_of_sample; i++){
      ...
      temp_current = qorc_ssi_adc_amps.getValue() - 6000;
    ...
    *p_accel_data++ = insPower;     //accel_data.z;
